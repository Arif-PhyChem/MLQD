...
def KRR(Xin: np.ndarray,
    ...
    tm = Xin.shape[1]
    if tm%2 != 0:
        t = np.linspace(init_time, init_time + time + (tm-1)*time_step, int(time/time_step)) 
    else:
        t = np.linspace(time_step + init_time, init_time + time + tm*time_step, int(time/time_step)) 
    nsteps = len(t)
    y = Xin;
...
def OSTL(Xin: np.ndarray,  
    ...
    #
    time_range = np.linspace(0, time, int(time/time_step))
...
def AIQD(Xin: np.ndarray,  
    ...
    #
    time_range = np.linspace(0,time,int(time/time_step)) # fs
    nsteps = len(t)
    y = np.zeros((nsteps, n_states**2), dtype=float)
    y1 = np.zeros((nsteps, n_states**2), dtype=complex)
